            ...
            if this["done"]:
                if this["responses"] is None:
                    ...
                    p.update(state="本轮已回应", closed=True, since=mtime(f"{d}/r{cur}-responses.md"),
                             cycle_note="写手已回应；有 accepted 改动则写手开下一轮，否则周期结束")
            else:
...
def render_cycle(p):
    ...
    top = (f'<span class="lab">当前周期</span><span><code style="font-weight:600">@ {esc(target)}</code></span>'
