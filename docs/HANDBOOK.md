...
# 每次退出（不论退出码）都刷新看板：状态只在脚本退出时变化。看板是只读的派生视图，缺席不影响流程。
# shellcheck disable=SC2064
trap "[ -n '${REVIEW_BOARD}' ] && command -v '${REVIEW_BOARD}' >/dev/null && '${REVIEW_BOARD}' --quiet 2>/dev/null; true" EXIT
...
.mast{display:flex;align-items:baseline;gap:28px;padding:10px 24px 9px;background:#1c1c1a;color:#e8e6e1;border-bottom:4px solid #c8375a}
.mast .brand{font-weight:700;font-size:14px;letter-spacing:.02em;color:#fff}
.mast .agents{display:flex;gap:22px;font-size:12px;color:#c9c7c1}.mast .ag b{color:#fff;font-weight:600;margin-right:4px}
.mast .agents .mute{color:#8a8883}
.mast .gen{margin-left:auto;font-size:12px;color:#8a8883;font-variant-numeric:tabular-nums}
...
.badge.rv{background:#dfeaf7;color:#1d4f8c}
.badge.wr{background:#f6e7c4;color:#6b4d0f}
.badge.none{background:#ecebe7;color:#5f5e5a;font-weight:500}
...
.evbtn{display:inline-block;border:1px solid #c9c7c1;border-radius:3px;padding:0 6px;line-height:17px;font-size:11px;color:#3a3936;background:#fff;flex:none}
.evbtn::before{content:"▸ ";font-size:9px;color:#8a8883}details[open].ev .evbtn::before{content:"▾ "}
details.ev>summary:hover .evbtn{background:#ecebe7}
.evteaser{font-size:11px;color:#8a8883;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0}
...
.sev{display:inline-block;font-size:11px;font-weight:600;padding:0 6px;line-height:18px;border-radius:2px;border:1px solid transparent;color:#5f5e5a;background:#ecebe7}
.sev.blocking{color:#9a2a22;background:#fbecea;font-weight:700}
.sev.should{color:#8a4b08;background:#fbe9d0}
.sev.nit{color:#5f5e5a;background:#ecebe7}
.fstat.ok{color:#2f6b3a}.fstat.bad{color:#9a2a22;font-weight:600}.fstat.wait{color:#8a4b08}
...
.bhead{display:flex;gap:14px;align-items:baseline;padding:6px 10px;background:#ecebe7;border-radius:2px}
.bitem{padding:8px 10px 8px 22px;border-bottom:1px solid #f0efeb}
...
.bl .who{font-size:11.5px;color:#8a8883;padding-top:1px}
    ...
    for fid in ids:
        ...
        if v.get("claim"):
            ...
            claim_cell = f'<div class="claim">{esc(v["claim"])}</div>' + (
                ...
                f'<code class="evid">{esc(ev)}</code></details>' if ev else "")
