...
## 不变量
每条写成「必须 X，因为 Y」。这是本文件最有价值的部分。
把过往修复背后的规则写进来 —— 例如「状态变更必须走读 ref → 算 next →
同步回写 ref → setState，因为直接 setState 会丢掉同一 tick 内的连续输入」。

## 已知取舍
故意没做的抽象、选了 A 没选 B 的原因。防止评审方把有意为之当成缺陷。

## 已知缺陷
从 docs/review-backlog.md 摘过来，一条一行。防止每轮重复报同样的问题。
